...
- **Remove (Plaid Connection Lost):** A **trash icon** in the item header opens an "Are you sure?" confirmation modal; on Confirm, frontend calls **POST /api/plaid/remove-item** with `item_id`. Backend performs a full removal (BUG-3 fix): best-effort revokes the item at Plaid (`itemRemove`), archives a snapshot, deletes all account rows (transactions cascade), deletes the encrypted `plaid_tokens` row, and removes the item-status row. Contrast with **POST /api/plaid/disconnect**, which also revokes and deletes the token but keeps an item-status row in state `disconnected` so the UI can show the institution as "Plaid Connection Lost". Next refetch: that item group disappears. Plan may still reference those `plaid_account_id`s until user unlinks.
