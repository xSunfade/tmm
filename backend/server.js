...
async function syncTransactionsForItemLegacy(itemId, userId, options = {}) {
  const syncRunId = randomUUID();
  const backfillDays = Number.isFinite(Number(options.backfillDays))
    ? Number(options.backfillDays)
    : DEFAULT_BACKFILL_DAYS;
  const backfillStartDate = shiftIsoDateByDays(Math.max(0, backfillDays));
  const backfillEndDate = dateToIsoDate(new Date());
  const accessToken = await getToken(itemId, userId);
  let baseCursor = null;
  let nextCursor = null;
  let added = [];
  ...
  try {
    ...
    const lockResult = await acquirePlaidItemSyncLock({
      ...
      lockSeconds: Number(process.env.PLAID_SYNC_LOCK_SECONDS || 120)
    });
    ...
    if (!breakerCheck.allowed) {
      const breakerErr = new Error(`Plaid circuit breaker open: ${breakerCheck.breaker?.reason || 'upstream instability'}`);
    ...
    // Backfill recent days to capture late-posting corrections.
    ...
    try {
      ...
      console.error(`Failed to write sync-run failure log for ${itemId}:`, logErr.message);
    }
    try {
      ...
      console.error(`Failed to update plaid_item_status for ${itemId}:`, statusErr.message);
    }
    try {
      if (isPlaidFailureForBreaker(error)) {
        await recordPlaidCircuitFailure({
          reason: getPlaidErrorCode(error) || error?.code || 'plaid_sync_failure'
        });
      }
    } catch (breakerErr) {
      console.error(`Failed to update circuit breaker for ${itemId}:`, breakerErr.message);
    }
    try {
      if (lockAcquired) {
        await releasePlaidItemSyncLock({
          ...
          itemId,
          success: false,
          cooldownSeconds: PLAID_SYNC_COOLDOWN_SECONDS
        });
      }
    } catch (lockErr) {
      console.error(`Failed to release sync lock for ${itemId}:`, lockErr.message);
    }
    throw error;
  }
}
