...
**Status (2026-07-14):** Phase 0 ✅ complete · Phase 1 ✅ complete (1.5/1.6 landed via PR #31 — single ledger engine, checkpoint seeding per D3, drift-at-today) · Phase 2 ✅ code complete: 2.1–2.4, 2.6, 2.7, 2.8 done + Sheets repositioned as beta backup (impl-phases 2.7); 2.5 (Supabase Pro at launch; PITR deferred to first real Plaid invoice) needs founder action · `project-roadmap` Phase 3 (domain model + positions + schema v3) ✅ complete — resolves BUG-6 (Ticker assets now quantity × price with exact DCA) and SEC-10 (Finnhub key out of exports) · `project-roadmap` **Phase 4 (entitlements/billing/lifecycle/security — this doc's "Phase 3" money paths) ✅ COMPLETE** (2026-07-16: merged via PR #49; dev migration applied, Stripe test catalog seeded + legacy $5 price archived, admin flags set, CI secrets configured, CodeQL clean) · **Phase 5.1 staging ✅ DONE** (2026-07-17: `tmm-staging` `wekawukfpdqinesbltnx` rebuilt from `supabase/migrations/` alone, fingerprint-matched dev, RLS anon-test 21/21; legacy `backend/supabase/migrations/` deleted — single migration source).
...
## Phase 2 — Data and persistence hardening (1.5–2 weeks) ✅ COMPLETE (code) — 2.5 (Supabase Pro at launch; PITR deferred to first Plaid invoice) remains a founder ops task
...
| 2.5 | Supabase Pro base at launch; **PITR deferred to the first real Plaid invoice** + tested restore runbook (DATA-8) ◀ FOUNDER ACTION | [H] | 0.5 d | ops | Pro on before launch; PITR enabled + restore rehearsed once when trigger fires |
